...
pub fn initialize_uboot(config: &SystemConfig, config_dir: &Path) -> BakeryResult<()> {
    ...
    match config.architecture {
        Architecture::Arm64 => {
            copy_recursive(
                paths::boot_dir().join("u-boot/arm64_config.txt"),
                config_dir.join("config.txt"),
            )
            .whatever("unable to copy `config.txt`")?;
            copy_recursive(
                paths::boot_dir().join("u-boot/bin/u-boot-arm64.bin"),
        ...
        Architecture::Armhf => {
            copy_recursive(
                paths::boot_dir().join("u-boot/armhf_config.txt"),
                config_dir.join("config.txt"),
            )
            .whatever("unable to copy `config.txt`")?;
    ...
    copy_recursive(
        paths::boot_dir().join("u-boot/bin/boot.scr"),
